...
class AgentType(Enum):
    ...
    CONTENT_GENERATOR = "content_generator"  # 文案生成器
    CUSTOM = "custom"  # 自定义类型
...
class ContentGeneratorAgent(BaseAgent):
    ...
    def __init__(self, 
        ...
        config = AgentConfig(
            name="文案生成器",
            description="专业的文案内容生成工具，支持多种类型文案创作",
    ...
    def process(self, params: Dict[str, Any]) -> AgentResponse:
        ...
        Returns:
            AgentResponse: 文案生成结果
        """
        try:
            query = params.get('query', '')
            inputs = params.get('inputs')
            scenario_content = params.get('scenario_content')
            user = params.get('user', 'content_generator')
            ...
            # 构建查询
            full_query = self._build_generation_query(query, scenario_content)
    ...
    def _build_generation_query(self, query: str, scenario: Optional[str]) -> str:
        """构建文案生成查询"""
    ...
    def generate_batch(self, 
                      queries: List[str], 
                      scenario: Optional[str] = None) -> List[AgentResponse]:
        """批量生成文案
        ...
        Returns:
            List[AgentResponse]: 文案生成结果列表
        """
        results = []
        
        for i, query in enumerate(queries):
            result = self.process({'query': query, 'scenario_content': scenario})
