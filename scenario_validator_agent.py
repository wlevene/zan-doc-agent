...
class AgentType(Enum):
    ...
    SCENARIO_VALIDATOR = "scenario_validator"  # 场景验收器
    CUSTOM = "custom"  # 自定义类型
...
class ScenarioValidatorAgent(BaseAgent):
    ...
    def __init__(self, 
        ...
        config = AgentConfig(
            name="场景验收器",
            description="专业的场景内容验收工具，支持多维度质量检查",
            agent_type=AgentType.SCENARIO_VALIDATOR
        )
    ...
    def process(self, params: Dict[str, Any]) -> AgentResponse:
        ...
        try:
            ...
            scenario_to_validate = params.get('scenario_to_validate')
            user = params.get('user', 'scenario_validator')
            ...
            # 构建查询
            full_query = self._build_validation_query(query, scenario_to_validate)
    ...
    def _build_validation_query(self, query: str, scenario: Optional[str]) -> str:
        """构建验收查询"""
    ...
    def validate_batch(self, 
        ...
        for i, scenario in enumerate(scenarios):
            query = criteria or f"请对第{i+1}个场景进行验收"
            result = self.process({'query': query, 'scenario_to_validate': scenario})
